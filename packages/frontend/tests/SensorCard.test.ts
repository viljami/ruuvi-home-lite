...
describe('SensorCard', () => {
  ...
  describe('TimeFormatter.formatAge', () => {
    it('should format seconds correctly', () => {
      expect(TimeFormatter.formatAge(0)).toBe('now')
      expect(TimeFormatter.formatAge(30)).toBe('30s ago')
      expect(TimeFormatter.formatAge(59)).toBe('59s ago')
    })

    it('should format minutes correctly', () => {
      expect(TimeFormatter.formatAge(60)).toBe('1m ago')
      expect(TimeFormatter.formatAge(120)).toBe('2m ago')
      expect(TimeFormatter.formatAge(3599)).toBe('59m ago')
    })

    it('should format hours correctly', () => {
      expect(TimeFormatter.formatAge(3600)).toBe('1h ago')
      expect(TimeFormatter.formatAge(7200)).toBe('2h ago')
      expect(TimeFormatter.formatAge(86399)).toBe('23h ago')
    })

    it('should format days correctly', () => {
      expect(TimeFormatter.formatAge(86400)).toBe('1d ago')
      expect(TimeFormatter.formatAge(172800)).toBe('2d ago')
    })

    it('should handle negative values', () => {
      expect(TimeFormatter.formatAge(-10)).toBe('now')
    })
  })
  ...
  describe('rendering', () => {
    ...
    it('should not display humidity when null', () => {
      ...
      const humidityElement = cardWithoutHumidity.querySelector('.sensor-humidity')
    ...
    it('should display age correctly', () => {
      ...
      expect(ageElement?.textContent).toBe('1m ago')
    })
  ...
  describe('event handling', () => {
    ...
    it('should not make MAC clickable for non-admin users', () => {
      ...
      const macElement = nonAdminCard.querySelector('.sensor-mac') as HTMLElement
  ...
  describe('update', () => {
    ...
    it('should update age display', () => {
      ...
      const ageElement = card.querySelector('.sensor-age')
      expect(ageElement?.textContent).toBe('2m ago')
    })
